$(document).ready(function () {
  // Делегирование событий с использованием jQuery
  ...
  function displayUser(users, nickname) {
    const searchResults = $("#searchResults ul");
    searchResults.empty();
  ...
  //? Проверка наличия ключа selectUsersToInvite в sessionStorage
  function checkAndRemoveSelectedClass() {
    if (!sessionStorage.getItem("selectUsersToInvite")) {
  ...
  //todo Использование MutationObserver для отслеживания изменений в классе тега body
  const observer = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      if (mutation.attributeName === "class") {
        const bodyClassList = document.body.classList;
        if (!bodyClassList.contains("modal_open")) {
          sessionStorage.removeItem("selectUsersToInvite");
        }
      }
    });
  ...
  // Настройка наблюдателя для тега body
